 ...
 *   1. billDate > now
 *        Future bill. paid=true, expected=billDate.
 *   2. billDate >= lastDue
 *        New bill (started this cycle). paid=true, expected=nextDue.
 *   3. lastPayment >= advanceWindowStart
 *        Advance payment covering nextDue cycle. expected=nextNextDue.
 *   4. lastPayment >= paymentWindowStart
 *        Covers lastDue cycle. expected=nextDue.
 *   5. (none of the above)
 *        Overdue. paid=false, due=true, expected=lastDue.
 *
 * paid flag (cases 2–4) = now < (expected_date + almost.weeks)
...
(function (root, factory) {
  ...
  function (moment, fastSort) {
    ...
    function parseBill(bill, now, config) {
      ...
      // Case 2: new bill started this cycle
      ...
      // Case 3: advance payment — paid within almost.weeks of nextDue → covers nextDue cycle
      var advancePaid =
      ...
      // Case 4: payment covers lastDue cycle
      var lastDuePaid =
